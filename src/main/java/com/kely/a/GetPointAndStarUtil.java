...
public class GetPointAndStarUtil {
    ...
    public static void main(String[] args) {
        List<RtnCommon> rtn = new ArrayList<RtnCommon>();
        List<BigDecimal> scoreRateList = new ArrayList<BigDecimal>();
        scoreRateList.add(new BigDecimal("0.67"));
        scoreRateList.add(new BigDecimal("0.25"));
        scoreRateList.add(new BigDecimal("0.05"));
        scoreRateList.add(new BigDecimal("0.01"));
        ...
        starRateList.add(0);
        starRateList.add(0);
        BigDecimal totalScore = new BigDecimal(24);
     ...
     */
    public static void getScoreAndStar(List<RtnCommon> rtn, int index, List<Integer> rankList, List<BigDecimal> pointRateList, List<Integer> starRateList, BigDecimal totalScore){
        ...
        for (int i = index ; i < rankList.size() ; i++){
            ...
            if(repeatNum == 1){
                RtnCommon rc = new RtnCommon(pointRateList.get(i).multiply(totalScore),starRateList.get(i));
                rtn.add(rc);//沒并列排名的情况
                ...
                for (int j = 0 ; j < pointRateList.size() ; j++){
                    RtnCommon rc = new RtnCommon(rangeAvgScore,rangeTotalStar);
                    rtn.add(rc);//人数全并列
                }
